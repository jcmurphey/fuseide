...
public class BasicNodeValidator implements ValidationSupport {
	...
	private boolean includeUIMessage = true;

	public BasicNodeValidator() {
		includeUIMessage = true;
	}

	public BasicNodeValidator(boolean includeUIMessage) {
		this.includeUIMessage = includeUIMessage;
	}
    ...
    public ValidationResult validate(CamelModelElement selectedEP) {
		...
		if (selectedEP != null && selectedEP.getCamelContext() != null) { // TODO: check why camel context can be null?!?
			...
			validateDetailProperties(selectedEP, result);
			final Component component = PropertiesUtils.getComponentFor(selectedEP);
			for (Parameter prop : new ArrayList<>(PropertiesUtils.getComponentPropertiesFor(selectedEP))) {
				Object value = PropertiesUtils.getPropertyFromUri(selectedEP, prop, component);
	...
	private void checkFor(ValidationResult result, Parameter prop, Object value, final IValidator validator) {
		...
		if (!status.isOK()) {
			result.addError(status.getMessage() + getUIMessage(prop));
		}
	}

	private String getUIMessage(Parameter prop) {
		if (includeUIMessage) {
			return " Please select the element, then go to Advanced->" + prop.getGroup();
		} else {
			return "";
		}
	}
